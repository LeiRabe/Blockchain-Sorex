...

{
	...
	public UserChannelInfo(Socket socket, ConcurrentLinkedQueue<SimpleTextMessage> messageQueue)
	...
	public void run() {
		...
		while (state) {
			...
			try {
				...
				if (errors >= 5) {
					// ferme le tunnel
					System.out.println("Fermeture de connexion " + this.name + " erreur: " + e.getMessage());
					state = false;
	...
	public void sendBloc(Bloc b) {// sending bloc to clients
		try {
	...
	public void run() {
		...
		while (state) {
			...
			while (!messageQueue.isEmpty()) {
				...
				Bloc b = null;
				try {
					b = (Bloc)this.input.readObject();// lire bloc provenant de client
				...
				while(all.hasMoreElements()){
					...
					user.sendMessage(sm);
					user.sendBloc(b);//envoyer bloc aux clients
				}
			}
		}
	}
